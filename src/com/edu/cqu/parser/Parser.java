...
import com.edu.cqu.core.Problem;
...
public class Parser {
    ...
    private void parserConstraints() {
        problem.constraintsCost = new HashMap<>();
        problem.neighbors = new HashMap<>();
        List<Element>  constraintElements =  rootElement.getChild("constraints").getChildren();
        for (Element constraintElement : constraintElements) {
            String constrainName = constraintElement.getAttributeValue("reference");
            AgentPair agentPair = new AgentPair(constraintElement.getAttributeValue("scope"));
            constraintInfo.put(constrainName,agentPair);
        }
    ...
    private void processTuple(String tuple, String constraintName) {
        ...
        Map<Integer,int[][]> constraintCost = problem.constraintsCost.get(pair.former);
        if (constraintCost == null){
            constraintCost = new HashMap<>();
            problem.constraintsCost.put(pair.former,constraintCost);
        }
        constraintCost.put(pair.latter,latterConstraintCost);
        constraintCost = problem.constraintsCost.get(pair.latter);
        if (constraintCost == null){
            constraintCost = new HashMap<>();
            problem.constraintsCost.put(pair.latter,constraintCost);
        }
    ...
    private void parserDomain() {
        List<Element> domainElements = new LinkedList<>();
