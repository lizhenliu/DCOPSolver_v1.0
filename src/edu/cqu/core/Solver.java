...
public class Solver {
    ...
    public void solve(String agentDescriptorPath, String agentType, String problemPath, FinishedListener listener, boolean showConstraintGraph, boolean showPesudoTreeGraph){
    ...
    public void solve(String agentDescriptorPath, String agentType, String problemPath, FinishedListener listener, boolean showConstraintGraph, boolean showPesudoTreeGraph, AgentIteratedOverListener agentIteratedOverListener){
        ProblemParser parser = new ProblemParser(problemPath);
        Problem problem = parser.parse();
        AgentManager manager = new AgentManager(agentDescriptorPath,agentType,problem,listener,false);
        if (showPesudoTreeGraph){
    ...
    public void batchSolve(String agentDescriptorPath,String agentType,String problemDir,int validateTime,FinishedListener listener,ProgressChangedListener progressChangedListener){
        ...
        List<File> acceptProblemFiles = new LinkedList<>();
        for (File file : dir.listFiles()) {
            if (file.isDirectory()){
                continue;
            }
            if (file.getName().endsWith(".xml")){
                acceptProblemFiles.add(file);
            }
        }
        String[] problemPaths = new String[acceptProblemFiles.size()];
        for (int i = 0; i < acceptProblemFiles.size(); i++) {
            problemPaths[i] = acceptProblemFiles.get(i).getAbsolutePath();
        }
